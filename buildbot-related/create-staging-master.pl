...
cd("tmp");
run("make -f Makefile.setup MASTER_NAME=sm-$username BASEDIR=$outdir PYTHON=python2.7 VIRTUALENV=virtualenv BUILDBOTCUSTOM_BRANCH=default BUILDBOTCONFIGS_BRANCH=default USER=$username MASTERS_JSON=$outdir/config.json virtualenv deps install-buildbot master master-makefile");
